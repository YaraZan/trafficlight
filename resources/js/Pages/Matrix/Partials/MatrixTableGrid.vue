...
const params = [
    ...
    'Нагрузка максимальная, кг',
    'Нагрузка минимальная, кг',
    'Температура масла в ГБ, °С',
    'Давление в ГН , МПа',
    'Давление в ГЦ , МПа',
    'Давление в ПГА, МПа',
    'Ток, А', 'Напряжение, В',
...
<template>
    <div class="w-full grid grid-cols-[19]">
        <div v-for="(item, index) in Array.from({ length: 19 })"
             class="p-1 block"
             :key="index"
            >
            <span class="text-[13px] font-semibold text-gray-800 dark:text-gray-300">{{ item }}</span>
        </div>
    </div>
