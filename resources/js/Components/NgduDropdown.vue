...
<template>
    <div class="relative flex items-center flex-col">
        ...
        <div :class="show ? 'visible' : 'invisible'" class="absolute top-12 p-3 right-0 z-10 bg-white rounded-lg shadow min-w-[300px]">
            ...
            <ul class="mt-[10px] gap-2 w-full flex flex-col">
                <li v-for="(ngdu, index) in ngdus" :key="index" class="flex items-center gap-2">
                    <input class="text-green-600 bg-gray-100 border-gray-300 rounded focus:ring-green-500" :id="`filter_option_${index}`" @change="filter" type="checkbox" :value="ngdu.Id">
                    <label :for="`filter_option_${index}`">{{ ngdu.NgduName }}</label>
                </li>
            </ul>
        </div>
    </div>
